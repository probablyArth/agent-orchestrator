...
describe.skipIf(!tmuxOk)("CLI-Core integration (hash-based architecture)", () => {
  ...
  it("backwards compatibility: legacy config with dataDir still works", async () => {
    const legacyDataDir = join(tmpDir, ".ao-sessions-legacy");
    const legacySessionName = `${sessionPrefix}-legacy-1`;

    // Create legacy flat structure
    mkdirSync(legacyDataDir, { recursive: true });
    writeFileSync(
      join(legacyDataDir, legacySessionName),
      `worktree=${tmpDir}\nbranch=feat/legacy\nstatus=working\nproject=test-project\nissue=LEGACY-1\n`,
    );

    // Config with explicit dataDir (legacy mode)
    const legacyConfig: OrchestratorConfig = {
      dataDir: legacyDataDir, // Explicit dataDir triggers legacy mode
      port: 3000,
      defaults: {
        runtime: "tmux",
        agent: "claude-code",
    ...
    const registry = createPluginRegistry();
    const sessionManager = createSessionManager({ config: legacyConfig, registry });

    // Should find session in legacy flat directory
    const sessions = await sessionManager.list("test-project");
    const legacySession = sessions.find((s) => s.id === legacySessionName);

    expect(legacySession).toBeDefined();
    expect(legacySession?.branch).toBe("feat/legacy");
    expect(legacySession?.issueId).toBe("LEGACY-1");
  });
});
